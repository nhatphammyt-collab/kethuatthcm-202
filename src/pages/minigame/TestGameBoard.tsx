...
import { Settings, MousePointer2, X, Copy, Check, ListChecks, RefreshCw, BookOpen, Info, Zap, Trophy, Award, Gift } from 'lucide-react';
  ...
  if (showGameEnd) {
    ...
    const rewardSummary = rewardConfig
      .map(({ key, label, accent }) => {
        const reward = mockRoomForEnd.rewards?.[key];
        if (!reward) return null;
        const claimedNames = (reward.claimedBy || []).map((pid) => mockRoomForEnd.players?.[pid]?.name || pid);
        return {
          key,
          label,
          accent,
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 relative overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 py-12">
          ...
          {rewardSummary.length > 0 && (
            <div className="max-w-4xl mx-auto mb-12">
              <h2 className="text-3xl font-bold text-white text-center mb-6 drop-shadow-lg">
                THỐNG KÊ PHẦN THƯỞNG (TEST VIEW)
              </h2>
